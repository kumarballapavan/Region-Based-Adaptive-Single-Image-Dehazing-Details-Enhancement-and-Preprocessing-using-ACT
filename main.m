function Odehazed_img = main(Oimg, GT, count, validation, imagename, output_folder)
...
img = im2double(Oimg);
imgg = im2double(Oimg);
img1=zeros(size(img));T=0.20;U=0.75;%img1f=zeros(size(img));
...
White=ones(size(img));
img1(img<T)=img(img<T);
img1(img>T)=White(img>T);
% disp('img(1,1,1):');disp(img(1,1,1));
 [m,n,p] = size(img);
%img1f = medfilt3(img1,[5 5 3]);
img1f=img1;
%figure;imshow([img1 img1f]);title('Dark overview vs dark filtered');
% img1fmean=mean(img1f(img1f<T));
% img1(img1f<T)=img(img1f<T);
% imgh(img1f>T)=img(img1f>T);
% img1(img1f>T)=0;
% % imgh(img1f<T)=0;
% M=mean(img1f(img1f<T));
% img1g=rgb2gray(img1);

% img1fg=rgb2gray(img1f);
%figure;imshow([img1 imgh]);title('Img1 vs Imgh');
%colormap jet;
img1=img1f;
% img1e=im2uint8(img1);
...
figure(1);imshow([rgb2gray(img1) rg]);title('Dark regions in hazy image vs enhanced dark regions');
colormap jet
% imwrite([rgb2gray(img1) rg],[imagename, '.png'])
pause(3);
end
...
[dehazed_img3, comp_time3, trans_map3] = fcn_multi(img,3);
...
imwrite(Odehazed_img,outputFileName);
 figure(5);imshow([Oimg Odehazed_img]);title('hazy dehazed');
%drawnow;


% Exec_time=toc;

% filename = 'Adaptive_Dehazing_Results_part2.xlsx';
% A = {'SSIM','PSNR','FADE ip','FADE op','BRISQUE','NIQE','Execution Time'; ssim_metrix_value,PSNR,FADE1,FADE2,brisque_value,sseq_value,Exec_time};
...
% imshow([GT Oimg Odehazed_img]);title('GT hazy dehazed');
% drawnow;
